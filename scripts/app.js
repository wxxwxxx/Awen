...
function makeConf(params) {
  ...
  try {
    let pu = {
      ...
      host: 'https://raw.githubusercontent.com/wxxwxxx/Rules/master/Auto/HOST.conf',
      ...
      mitm: 'https://raw.githubusercontent.com/wxxwxxx/Rules/master/Surge/MITM.conf',
      ...
      function ssr2ss(proxies) {
        ...
        let res = proxyList.map(proxy => {
          if (/=\s*custom/.test(proxy)) {
            return proxy.replace(/=\s*custom/g, '= ss').replace(/\s*(none|aes|rc4|salsa20|chacha20)[^,$]+/g, 'encrypt-method=$&').replace(/(?![0-9]+$)(?![a-zA-Z]+$)[0-9A-Za-z_]{11,16}/g, 'password=$&').replace(/\, undefined/g, '')
          }         
